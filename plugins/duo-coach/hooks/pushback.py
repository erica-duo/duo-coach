#!/usr/bin/env python3
"""Duo pushback hook — don't let Claude give up.
...
digging — the same way Erica would respond. Capped at 2 pushbacks per
session so it can never loop forever.
...
PUSHBACK_FIND = (
    "Duo pushback — \"not found\" isn't a final answer yet. Try all of these first:\n"
    "\n"
    "- Alternate spellings and name variants (first name, full name, business name — Slack and Notion often differ)\n"
...
PUSHBACK_DO = (
    "Duo pushback — you have more tools than you think (files, shell, web, every MCP server). "
...
PUSHBACK_FINAL = (
    "Duo pushback, final round — re-read your last answer like a skeptical client would. "
...
def main():
    ...
    counter_file = os.path.join(tempfile.gettempdir(), f"duo-pushback-{session_id}")
    try:
